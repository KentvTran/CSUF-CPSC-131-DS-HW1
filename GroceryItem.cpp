...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
  if( _upcCode != rhs._upcCode ) return ( _upcCode <=> rhs._upcCode );
  if( _productName != rhs._productName ) return ( _productName <=> rhs._productName );
  if( _brandName != rhs._brandName ) return ( _brandName <=> rhs._brandName );
  if( _price < rhs._price ) return std::weak_ordering::less;
  if( rhs._price < _price ) return std::weak_ordering::greater;
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  std::string upc, product, brand;
  char        comma;
  double      price;
  if( stream >> std::quoted( upc ) >> comma >> std::quoted( brand ) >> comma >> std::quoted( product ) >> comma >> price )
  {
    groceryItem.upcCode( std::move( upc ) );
    groceryItem.productName( std::move( product ) );
    groceryItem.brandName( std::move( brand ) );
